...
var _ = Describe("uaa_registrar", func() {
	...
	It("new should fetch auth token", func() {
		called := false
		tokenFetcher.RefreshAuthTokenFn = func() (string, error) {
			called = true
			return "my-token", nil
		}

		_, err := NewUaaRegistrar(
			"https://uaa.example.com", tokenFetcher, true, logger,
		)
	...
	It("new should return error", func() {
		...
		registrar, err := NewUaaRegistrar(
			testServer.URL, tokenFetcher, true, logger,
		)
...
func (m *MockTokenFetcher) RefreshAuthToken() (string, error) {
